...
class SubtitlerField extends BaseField {
  ...
  public function headline() {
    ...
    if(!$this->readonly && (is_null($this->limit) || (is_int($this->limit) && $entries->count() < $this->limit))) {
      ...
      $add->addClass('structure-add-button label-option add-button-hide');
      $add->data('modal', true);
      $add->attr('href', purl($this->model, 'field/' . $this->name . '/subtitler/add'));
  ...
  public function content() {
  	if (isset($this->src)) {
  		$src = $this->src();
  		$file = $this->page()->content()->get($src)->toFile();
  		$type = 'page';
  	}
  	else {
  		$file = $this->file();
    ...
    return tpl::load(__DIR__ . DS . 'template.php', array('field' => $this, 'file' => $file, 'type' => $type));
  }

  public function url($action) {
    return purl($this->model(), 'field/' . $this->name() . '/subtitler/' . $action);
  }  
